# -*- coding: utf-8 -*-
"""
Created on Wed Jul 28 14:13:33 2021
...
pickle_in = open('equilibrium.pickle', 'rb')
eqb_states = pickle.load(pickle_in)
pickle_in.close()

"""
Switch off equilibrating steps
Make the averging steps only use the data from eqb_states
"""
...
class MonteCarlo:
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--sites', type=int, metavar='',
                                 help='Total number of sites including nanopores and interlayers', default=30)
        self.parser.add_argument('--sps', type=int, metavar='',
                                 help='Number of Monte Carlo steps per site', default=500)
        self.parser.add_argument('--sps2', type=int, metavar='',
                                 help='Number of steps for averaging after equilibration', default=1000)
        ...
        self.parser.add_argument('--index', type=int, metavar='',
                                 help='Used to determine which row to save data to in results_array')
        ...
        """Open the pickle jar. The jar of pickled lattices."""
        pickle_in = open('equilibrium.pickle', 'rb')
        eqb_states = pickle.load(pickle_in)
        pickle_in.close()
        self.lattice = copy.deepcopy(eqb_states[self.args.index])
    ...
    def plot_results(self, results_array, avrN_data, avrU_data): #this is now only for saving CSV
        """
        ...
        results_df = results_df.replace(0, np.nan).dropna(axis=0, how='all')  # For the rows with all '0' entries they are replaced with 'nan' and then these rows are dropped.
        results_df = results_df.replace(np.nan, 0)  # As some legitimate 0 entries such as 0 volts we flip back the remaining from 'nan' to 0.
        ...
        uid = dt.datetime.now().strftime("%d/%m/%Y %H:%M:%S") # The date and time is appended to the file name
        uid = uid.replace('/', '-')
        uid = uid.replace(' ', '_')
        uid = uid.replace(':', '-')
        ...
        """
        Need to add a couple of lines to make this HEC friendly
        """
    ...
    def run_simulation(self):
        ...
        steps = self.args.sps * self.args.sites # This is for equilibration steps
        steps2 = self.args.sps2 * self.args.sites # This is for averaging steps
        average_rows = steps / self.args.sample_frequency
        average_rows2 = steps2 / self.args.sample_frequency
        data_array = np.zeros((6, int(average_rows2)))  # Records the parameters at the sample frequency
        ...
        mu = self.args.mu_list[self.args.index]
        ...
        """
        Switching off any equilibrating steps
        """
        ...
        data_count = 0
        for i in range(steps2): #use a different step variable for this part
            self.monte_carlo(mu)
        ...
        try: # Check if picked file exists, if not define a results_array filled with 0s
            pickle_in = open('results_array.pickle', 'rb')
            results_array = pickle.load(pickle_in)
            pickle_in.close()
        except:
        ...
        if variance != 0:
            ...
            results_array[self.args.index, 0] = mean_x1
            results_array[self.args.index, 1] = mean_x2
            results_array[self.args.index, 2] = mean_x
            results_array[self.args.index, 3] = mu * -1
            results_array[self.args.index, 4] = delta_entropy  # Partial molar entropy
            results_array[self.args.index, 5] = variance / (self.kb * self.T * self.args.sites)  # dq/de
            results_array[self.args.index, 6] = cov_un / variance  # Partial molar enthalpy

        
        # Now pickle the equilibrium states
        # Syntax for pickling was learnt from the YouTube channel sentdesk
        pickle_out = open('results_array.pickle', 'wb')
        pickle.dump(results_array, pickle_out)
        pickle_out.close()
        # The pickle is saved in the script directory
        
        """
        RAN INTO ISSUE HERE: THE RESULTS ARRAY THING CAN'T JUST BE PICKLED YET
        USE SELF.INDEX HERE
        What row_count does in results_array: as it iterates, the data for each chemical potential is 
        saved in a different row. As the code originally stood, the chemical potentials are iterated through
        in order, and the results are saved in results_array in order. 
        
        If I use self.index instead of row_count, the data will still be saved in order.
        
        After the code saves data for one equilibrium state in results_array, the array will need to be 
        pickled. 
        
        Use an if/else statement: If the pickled file does not exist, then initialise results_array
        If the pickled file does exist, load it in and assign it to results_array
        
        When running the code, make sure that you enter the same number of sites as that used in the pickled
        file
        """

        if self.args.index == 58: # If at final index, a csv. file should be generated
